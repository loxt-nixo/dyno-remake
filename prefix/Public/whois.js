...
module.exports = {
    ...
    execute: async function (msg, client, args) {
        let member = msg.guild.members.cache.get(args[0]) || msg.mentions.members.first() || msg.member;

		if (!member) return msg.channel.send({ embeds: [new EmbedBuilder().setColor(client.config.embedError).setDescription(`<:error:1205124558638813194> I couldn't find the user ${member}`)], ephemeral: true });
