...
const client = new Client({
  ...
  partials: ["GuildMembers", "Messages"],
});
...
client.checkMod = async function checkMod(roles, guild) {
  const guildConfig = await client.schemas.GuildConfig.findOne({
    Guild: guild,
  });
  ...
  setInterval(async () => {
    ...
    for (const giveaway of giveaways) {
      if (Date.now() >= giveaway.EndsAt) {
        ...
        channel.send({
          content: `Congratulations ${text}, you won ${giveaway.Prize}!`,
        });
...
client.on(Events.GuildMemberAdd, async function (member) {
  ...
  let embed = new EmbedBuilder()
    ...
    .addFields({
      name: `Account Age`,
      value: `${await formatUnix(member.user.createdAt / 1000)}`,
    })
    .setColor("#43b582")
...
client.on(Events.MessageUpdate, async function (oldMessage, newMessage) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      {
        name: `Before`,
        value: `${oldMessage.content || "*No message content*"}`,
      },
      {
        name: `after`,
        value: `${newMessage.content || "*No message content*"}`,
      }
    )
...
client.on(Events.MessageBulkDelete, async function (messages) {
  ...
  const embed = new EmbedBuilder()
    .setColor("#337fd5")
    .setAuthor({
      name: `${messages.first().guild.name}`,
      iconURL: messages.first().guild.iconURL(),
    })
    .setTimestamp()
    .setDescription(
      `**Bulk Delete in ${messages.first().channel}, ${
        messages.size
      } messages deleted**`
    );
...
client.on(Events.VoiceStateUpdate, async function (oldState, newState) {
  ...
  if (!oldState.channel && newState) {
    embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${newState.member} joined voice channel ${newState.channel}**`
      )
      .setFooter({ text: `ID: ${newState.member.id}` })
      .setTimestamp();

    guildConfig = await client.schemas.GuildConfig.findOne({
  ...
  if (oldState.channel && newState.channel) {
    embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${newState.member} switched voice channels ${oldState.channel} -> ${newState.channel}**`
      )
      .setFooter({ text: `ID: ${oldState.member.id}` })
      .setTimestamp();
...
client.on(Events.GuildMemberUpdate, async function (oldMember, newMember) {
  ...
  const removedRoles =
    client.cache.get(
      `removedRoles_${newMember.user.id}_${newMember.guild.id}`
    ) || [];
  const addedRoles =
    client.cache.get(`addedRoles_${newMember.user.id}_${newMember.guild.id}`) ||
    [];
  ...
  client.cache.set(
    `addedRoles_${newMember.user.id}_${newMember.guild.id}`,
    addedRoles
  );

  await new Promise((resolve) => setTimeout(resolve, 1000 * 3));

  const remRoles = client.cache.get(
    `removedRoles_${newMember.user.id}_${newMember.guild.id}`
  );
  const addRoles = client.cache.get(
    `addedRoles_${newMember.user.id}_${newMember.guild.id}`
  );
  ...
  if (addRoles?.length > 0) {
    if (addRoles?.length === 1) {
      ...
      embed = new EmbedBuilder()
        ...
        .setDescription(
          `**${newMember} was given the roles ${addedRoles.join(", ")}**`
        )
        .setFooter({ text: `ID: ${newMember.id}` })
        .setTimestamp();
    }
  ...
  if (remRoles?.length > 0) {
    if (remRoles?.length === 1) {
      embed = new EmbedBuilder()
        ...
        .setDescription(
          `**${newMember} was removed from the ${removedRoles[0]} role**`
        )
        .setFooter({ text: `ID: ${newMember.id}` })
        .setTimestamp();
    } else {
      embed = new EmbedBuilder()
        ...
        .setDescription(
          `**${newMember} was removed from the roles ${removedRoles.join(
            ", "
          )}**`
        )
        .setFooter({ text: `ID: ${newMember.id}` })
...
client.on(Events.GuildRoleUpdate, async function (oldRole, newRole) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `**Role Color Changed: #${oldRole.color.toString(
        16
...
client.on(Events.GuildEmojiUpdate, async function (oldEmoji, newEmoji) {
  ...
  const embed = new EmbedBuilder()
    .setColor("#337fd5")
    .setDescription(
      `\`:${oldEmoji.name}:\` was changed to \`:${newEmoji.name}:\``
    )
